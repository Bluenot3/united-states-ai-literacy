        ...
        <div className="my-8 p-6 bg-brand-bg rounded-2xl shadow-neumorphic-out">
            <div className="flex flex-col md:flex-row items-center justify-center gap-8">
                ...
                <div className="flex flex-col items-center gap-3">
                    <button onClick={() => move(0, -1)} disabled={isFinished || loading} className="w-16 h-12 rounded-lg shadow-neumorphic-out disabled:opacity-50 flex items-center justify-center">▲</button>
                    <div className="flex gap-3">
                        <button onClick={() => move(-1, 0)} disabled={isFinished || loading} className="w-16 h-12 rounded-lg shadow-neumorphic-out disabled:opacity-50 flex items-center justify-center">◀</button>
                        <button onClick={() => move(1, 0)} disabled={isFinished || loading} className="w-16 h-12 rounded-lg shadow-neumorphic-out disabled:opacity-50 flex items-center justify-center">▶</button>
                    </div>
                    <button onClick={() => move(0, 1)} disabled={isFinished || loading} className="w-16 h-12 rounded-lg shadow-neumorphic-out disabled:opacity-50 flex items-center justify-center">▼</button>
                </div>
