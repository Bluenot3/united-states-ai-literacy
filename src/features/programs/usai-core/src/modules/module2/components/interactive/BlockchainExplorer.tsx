...
const BlockchainExplorer: React.FC<InteractiveComponentProps> = ({ interactiveId }) => {
    ...
    const handleSelect = (item: Block | Transaction) => {
                    ...
                    <div className="space-y-3">
                        {chain.map(block => (
                            <div key={block.id} className={`p-3 bg-brand-bg rounded-lg transition-all duration-300 ${selectedItem && 'prevHash' in selectedItem && selectedItem.id === block.id ? 'shadow-neumorphic-in' : 'shadow-neumorphic-out'}`}>
                                <button onClick={() => handleSelect(block)} className="font-semibold text-brand-primary w-full text-left">Block #{block.id}</button>
                                <div className="mt-2 space-y-1">
                                    {block.transactions.map(tx => (
                                        <button key={tx.id} onClick={() => handleSelect(tx)} className={`text-sm w-full text-left pl-4 rounded py-1 ${selectedItem && 'from' in selectedItem && selectedItem.id === tx.id ? 'text-brand-primary font-semibold' : 'text-brand-text-light hover:text-brand-primary'}`}>Tx: {tx.id.substring(0,8)}...</button>
                                    ))}
                                </div>
                            </div>
                        ))}
