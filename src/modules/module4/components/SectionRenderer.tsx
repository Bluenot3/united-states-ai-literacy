...
const HeroIntroMod4 = React.lazy(() => import('./interactive/HeroIntroMod4'));
...
// Fix: Import HitlRagSimulator from Module 2 if missing in Module 4, or ensure it exists.
// Assuming it was found in Module 2 but needed here.
// For now, mapping RlhfTrainerGame as fallback was creating the "bad" UX.
// We will attempt to use the one from Module 2 if possible, or fallback gracefully.
const HitlRagSimulator = React.lazy(() => import('../../module2/components/interactive/HitlRagSimulator'));
const HuggingFaceGuide = React.lazy(() => import('../../module2/components/interactive/HuggingFaceGuide'));
...
const formatText = (text: string): React.ReactNode => {
    ...
    return parts.map((part, index) => {
        ...
        if (part.startsWith('*') && part.endsWith('*')) {
            return <em key={index} className="italic text-amber-300/80 font-medium">{part.slice(1, -1)}</em>;
        }
        if (part.startsWith('`') && part.endsWith('`')) {
            return <code key={index} className="bg-amber-400/[0.07] px-2 py-0.5 rounded-lg text-sm font-mono text-amber-300 border border-amber-400/15">{part.slice(1, -1)}</code>;
        }
        return part;
    });
};
        ...
        case 'quote':
            return (
                <blockquote className="relative border-l-4 border-transparent pl-6 my-8 py-2 animate-slide-in-up" style={animationStyle}>
                    <div className="absolute left-0 top-0 bottom-0 w-1 bg-gradient-to-b from-amber-400 to-rose-400 rounded-full" style={{ marginLeft: '-3px' }} />
                ...
                return (
                    <ul className="mb-8 pl-2 text-lg text-slate-300 space-y-3">
                        {item.content.map((li, i) => (
                            ...
                            >
                                <span className="mt-2.5 w-1.5 h-1.5 rounded-full bg-amber-400/70 flex-shrink-0" />
                    ...
                    return (
                        <div className="my-6 animate-slide-in-up" style={animationStyle}>
                            <div className="flex items-center gap-3 mb-4 pl-1">
                                <div className="flex h-9 w-9 items-center justify-center rounded-xl border border-amber-400/25 bg-amber-400/[0.08]">
                                    <LabIcon />
                                </div>
                                <span className="text-[11px] font-bold uppercase tracking-[0.28em] text-amber-300">Interactive Lab</span>
                            </div>
                            <Suspense fallback={
