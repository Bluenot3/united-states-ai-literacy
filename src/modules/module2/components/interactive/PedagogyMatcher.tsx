...
const PedagogyMatcher: React.FC<InteractiveComponentProps> = ({ interactiveId }) => {
    ...
    React.useEffect(() => {
        if(selectedTerm && selectedDef) {
            ...
            if(termObj && termObj.def === selectedDef) {
                const newCorrectMatches = [...correctMatches, selectedTerm];
                setCorrectMatches(newCorrectMatches);
                setFeedback('Correct Match!');
                
                if (newCorrectMatches.length === pedagogies.length && user && !user.progress.completedInteractives.includes(interactiveId)) {
                    addPoints(25);
    ...
    }, [selectedTerm, selectedDef, correctMatches, user, addPoints, updateProgress, interactiveId]);
