...
// Detect Part 2 boundary for visual separator
const isPart2Start = (section: Section): boolean => {
    const t = section.title.toLowerCase();
    return t.includes('part 2') || t.includes('section 2') || t.includes('advanced') && t.includes('decision') || t.includes('second mind');
};
...
const MainContent: React.FC<MainContentProps> = ({ title, sections, sectionRefs, visibleSections, unlockedSectionIds, hiddenSectionIds }) => {
    const { buckets, rows } = useContentOverrides('vanguard', 'module3');
                ...
                <React.Fragment key={section.id}>
                    {showPart2Divider && (
                        <div className="relative my-14 flex items-center gap-5">
                            ...
                            <div className="flex items-center gap-3 rounded-full border border-emerald-400/25 bg-emerald-400/[0.08] px-5 py-2.5 backdrop-blur-sm">
                                <span className="h-1.5 w-1.5 rounded-full bg-emerald-400 shadow-[0_0_6px_rgba(52,211,153,0.8)]" />
                                <span className="text-[11px] font-bold uppercase tracking-[0.32em] text-emerald-300">Part 2</span>
                            </div>
                            <div className="h-px flex-1 bg-gradient-to-r from-transparent via-emerald-400/40 to-transparent" />
                        </div>
                    )}
                        ...
                        <div className="relative">
                            ...
                            {section.subSections && (
                                <div
                                    ...
                                    style={{ borderImage: 'linear-gradient(to bottom, rgba(52,211,153,0.28), rgba(20,184,166,0.18), rgba(15,23,42,0.04)) 1' }}
                                >
    ...
    return (
        <div className="py-8">
            <header className="mb-10 overflow-hidden rounded-[1.5rem] border border-emerald-400/20 bg-[linear-gradient(145deg,rgba(6,11,24,0.94)_0%,rgba(13,22,42,0.9)_60%,rgba(7,14,30,0.94)_100%)] p-6 shadow-[0_24px_55px_rgba(2,6,23,0.42)] backdrop-blur-xl md:p-8">
                <div className="flex flex-wrap items-center gap-3">
                    <span className="inline-flex items-center rounded-full border border-emerald-400/25 bg-emerald-400/[0.10] px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.28em] text-emerald-200">
                ...
                <div className="mt-6 grid gap-3 md:grid-cols-3">
                    {signalCards.map((card) => (
                        <div key={card} className="rounded-2xl border border-white/10 bg-white/[0.05] px-4 py-4 backdrop-blur-sm">
                            <p className="text-[11px] font-semibold uppercase tracking-[0.26em] text-emerald-300">Signal</p>
                            <p className="mt-2 text-sm font-semibold leading-6 text-slate-100">{card}</p>
                        </div>
                    ))}
                </div>
                ...
                <div className="mt-7 flex items-center gap-3">
                    <div className="flex items-center gap-3 rounded-full border border-emerald-400/20 bg-emerald-400/[0.06] px-4 py-2">
                        <span className="h-1.5 w-1.5 rounded-full bg-emerald-400 shadow-[0_0_6px_rgba(52,211,153,0.8)]" />
                        <span className="text-[11px] font-bold uppercase tracking-[0.32em] text-emerald-300">Part 1 — The Architecture of a Second Brain</span>
                    </div>
                </div>
            </header>
