...
const CommandPalette: React.FC<CommandPaletteProps> = ({ isOpen, onClose, sections }) => {
    ...
    return (
        <div
            ...
            className="fixed inset-0 z-50 flex justify-center pt-20 animate-fade-in bg-black/30 backdrop-blur-sm"
            onClick={onClose}
        >
            <div
                className="w-full max-w-xl h-fit bg-white/95 backdrop-blur-xl rounded-2xl shadow-2xl flex flex-col animate-slide-in-up border border-white/50"
                ...
                <ul id="command-palette-results" ref={resultsRef} className="max-h-96 overflow-y-auto p-2">
                    {filteredSections.length > 0 ? filteredSections.map((section, index) => (
                        <li
                            ...
                            className={`p-3 rounded-lg cursor-pointer flex justify-between items-center transition-colors ${index === activeIndex ? 'bg-emerald-500/10 text-emerald-600' : 'hover:bg-slate-100'
                                }`}
                        >
                            <div>
                                {section.parentTitle && <span className="text-xs text-slate-400">{section.parentTitle} / </span>}
                                <span className="font-semibold">{section.title}</span>
                            </div>
                            <span className="text-xs text-slate-400">Jump to &crarr;</span>
                        </li>
                    )) : (
