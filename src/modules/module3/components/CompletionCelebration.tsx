...
const launchConfetti = () => {
    ...
    for (let i = 0; i < confettiCount; i++) {
        ...
        const colors = ['#10b981', '#14b8a6', '#059669', '#fde047', '#f97316'];
...
const CompletionCelebration: React.FC = () => {
    ...
    const handleDownload = async (format: 'jpg' | 'pdf') => {
        ...
        if (format === 'jpg') {
            ...
            link.download = 'Zen_Vanguard_Certificate_M3.jpg';
            ...
            pdf.save('Zen_Vanguard_Certificate_M3.pdf');
        }
    };

    const certId = `ZV-M3-${user?.email.slice(0, 4).toUpperCase()}${new Date().getFullYear()}`;
        ...
        <div className={`fixed inset-0 z-50 flex items-center justify-center transition-opacity duration-500 ${showModal ? 'opacity-100' : 'opacity-0'}`} style={{ backgroundColor: 'rgba(0,0,0,0.5)', backdropFilter: 'blur(8px)' }}>
            <div className="bg-brand-bg/80 backdrop-blur-xl p-8 rounded-2xl shadow-soft-xl max-w-4xl w-full mx-4 animate-slide-in-tilt">
                ...
                >
                    ...
                    <div className="absolute -bottom-1/4 -right-1/4 w-1/2 h-1/2 bg-teal-500/10 rounded-full blur-3xl"></div>
                    <div className="absolute inset-0 bg-grid-pattern opacity-5" style={{ '--grid-color': 'rgba(16, 185, 129, 0.2)', '--grid-size': '30px' } as React.CSSProperties}></div>
                    ...
                    <div className="text-center z-10 my-4">
                        ...
                        <p className="text-brand-text-light mt-2 max-w-xl mx-auto">for successfully completing Module 3: Personal Intelligence & Cognitive Systems.</p>
                    </div>
                    ...
                    <div className="flex justify-between items-end text-xs font-mono text-brand-text-light/80 z-10">
                        ...
                        <div className="text-right">
                            ...
                            <p>SECTIONS_COMPLETED: {user?.modules[3]?.completedSections.length || 0}</p>
                        </div>
                    </div>
                </div>
                ...
                <div className="mt-6 flex flex-col sm:flex-row items-center justify-center gap-4">
                    ...
                    />
                    <button onClick={() => handleDownload('jpg')} className="px-6 py-2 bg-white/80 rounded-lg shadow-soft-lg font-semibold text-emerald-600 hover:bg-white transition-all">Download JPG</button>
                    <button onClick={() => handleDownload('pdf')} className="px-6 py-2 bg-white/80 rounded-lg shadow-soft-lg font-semibold text-emerald-600 hover:bg-white transition-all">Download PDF</button>
                </div>
            </div>
        </div>
    );
